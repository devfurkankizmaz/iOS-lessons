...
func showBMIStatus(bmiArr: [(name: String, bmi: Double)]) {
    ...
    bmiArr.forEach { user in
        ...
        case 0..<18.5:
            ...
            delegate.start("\(user.name) : Zayıf")
        case 18.5..<25:
            // print("\(bmi.name) : Normal")
            delegate.start("\(user.name) : Normal")
        case 25..<30:
            // print("\(bmi.name) : Fazla Kilolu") // Overweight
            delegate.start("\(user.name) : Fazla Kilolu")
        case 30...:
            // print("\(bmi.name) : Obez") // Obese
            delegate.start("\(user.name) : Obez")
        default:
            // print("\(bmi.name) : Invalid BMI value")
            delegate.start("\(user.name) : Invalid BMI value")
        }
    }
}
